...
/*******************************************************************************
 ** Meta Data Producer for SFTPExportSourceFile
 *******************************************************************************/
public class SFTPExportDestinationFileBackendMetaDataProducer extends QBitComponentMetaDataProducer<QBackendMetaData, SFTPDataIntegrationQBitConfig>
{
   private static final QLogger LOG = QLogger.getLogger(SFTPExportDestinationFileBackendMetaDataProducer.class);

   public static final String NAME = "SFTPExportDestinationFileBackend";

   public static final String VARIANT_TYPE_KEY = SFTPExportConfig.TABLE_NAME;
   ...
   {
      boolean doProvideTable             = getQBitConfig().getDestinationFileTableConfig().getDoProvideTable();
      boolean isDestinationBackendThisBackend = NAME.equals(getQBitConfig().getDestinationFileTableConfig().getBackendName());

      return (doProvideTable && isDestinationBackendThisBackend);
   }
   ...
   public QBackendMetaData produce(QInstance qInstance) throws QException
   {
      ...
      backendMetaData.setBackendVariantsConfig(new BackendVariantsConfig()
         .withOptionsTableName(SFTPExportConfig.TABLE_NAME)
   ...
   public static class VariantRecordSupplier implements UnsafeFunction<Serializable, QRecord, QException>
   {
      ...
      {
         QueryOutput queryOutput = new QueryAction().execute(new QueryInput(SFTPExportConfig.TABLE_NAME)
